...
課題が `review` に進まず、`blocked` など想定外の状態になった場合は、まず Web UI のコメントを確認して、エージェントが報告した原因を特定してください。コメントだけで原因が分からない場合は、[blocked セッションの復旧](pathname:///guides/recover-blocked-session)が、対象の Codex 実行を続行する手掛かりになることがあります。
