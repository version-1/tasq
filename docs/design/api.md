...
`DELETE /api/v1/projects/{id}` deletes the project and all issue-tracker descendants owned by that project: issues, issue dependency edges that reference those issues, comments, attachment records and attachment files under `$TQ_HOME/system/data/attachments`, and stored project workflow overrides. It does not delete or modify the user's project directory recorded in `project.location` or any worktrees.
